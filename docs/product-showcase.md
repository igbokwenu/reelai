...
- Final-render videos, narration, music, and logos are fetched and signature-checked once before Chromium starts. They are then served from a loopback-only byte-range server during frame extraction, so local rendering does not repeatedly proxy media through the active Next.js request or remote storage. Uploaded logos still use Remotion's render-blocking image component, but decode from the staged local copy; an unavailable or invalid source therefore fails early with the specific media item named instead of timing out midway through the export.
