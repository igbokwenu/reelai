...
- User can choose Product Showcase, which requires exactly one product and one product image and creates a 5–15 second realistic or 3D showcase from three proposed creative directions. Its optional persisted Razzmatazz toggle instead fixes the output to one 5-second intact-product hero scene with no people or separation, explicit product motion, one animated surrounding background-energy effect, center-of-attention framing, and a short CTA. Both output modes support one optional logo and one project website; Brand Reel also keeps its one product-image slot optional.
...
- Razzmatazz locks the normal Product Showcase feasibility range to exactly one 5-second scene, `NO_PERSON`, `AVOID` separation, at most 10 narration words, and a 2–6 word final tagline/CTA. The creative validator requires the directed shot to state product motion, an active surrounding energy effect, and centered/sole-focus hero framing; Remotion exports exactly 150 frames at 30 fps.
...
- Validate new shot directions for generation safety as one 14–60 word mood-first sentence, at most one motivated two-beat progression, exactly one reliable camera move, no passive framing language, and a 5–10 second duration. Continue prompting for one focal action arc, a visible story change, layered composition, and varied camera behavior, but treat those subjective quality goals as advisory rather than finite-vocabulary blockers. Keep brand and continuity prose in anchor generation/reference chaining, not in the video prompt.
