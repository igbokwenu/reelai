...
1. User supplies a company website and, optionally, a short creative direction. Advanced project fields remain available but are not required for the primary flow. The direction is stored with the website source metadata, avoiding a project-schema migration for transient generation guidance.
   Product Showcase is an alternate intake lane: the user supplies exactly one named product and one hero image, optional details/logo, one optional project website, a 5/10/15-second target, and realistic or 3D style. Brand Reel uses the same initial identity-asset step but keeps both uploads optional. The project is created without starting research, assets are durably uploaded, then Brand Kit generation is queued so logo vision can influence the Working palette. A failed intake upload removes the incomplete project.
...
5. Concept generation derives explicit capabilities from uploaded sources. Without an uploaded logo, product image, UI screenshot/reference ad, or other visual source, prompts prohibit manufacturing those visual elements. Structured concepts are validated before image spend. Brand Reel generation asks for three different editorial engines—attention-to-purpose, proof-through-process, and a signature brand world adapted to the verified offer—rather than three hook variants. After one model schema-repair pass, a still-incomplete concept set is completed by a bounded local creative rescue that preserves substantive fields, supplies only safe missing planning language, locks project timing/scene count, and returns exactly three differentiated directions. A separate internal grounding recovery strips unsupported logo/product/UI/claim execution; if that remains unsafe, it substitutes the grounded premium routes before preview generation rather than asking the user to correct the agent. This remains internal in manual and Auto mode; the job records structured and grounding recovery methods for diagnostics. A single concept can also be regenerated with an optional concise adjustment note; that prompt includes the Brand Kit, verified evidence, the target being replaced, and the two retained concepts as anti-duplication context.
6. Preview prompts receive the same grounding constraints and an explicit opening-frame contract: each concept image is the intended Scene 1 still, not a mood-board thumbnail. Product images are ordered first and local-development artifacts are uploaded to QwenCloud temporary OSS rather than being silently dropped when `PUBLIC_APP_URL` is unavailable. Product-grounded generation runs in reference-edit mode, and QwenCloud vision compares the result with the supplied product references; a generic substitute, material identity drift, or failed local-reference resolution is blocked from selection. Single-concept replacement is atomic from the user's perspective: the retained concepts keep their IDs and previews, the replacement keeps its concept ID and selection state, and only an unreferenced superseded preview is cleaned up after persistence. If it drives a storyboard, that storyboard and its scenes return to draft review while prior production artifacts remain durable as selectable history.
...
8. Storyboard generation begins with a capability preflight against the selected concept. Missing visual references are treated as adaptation constraints, not upload blockers: the first prompt preserves strategy while replacing unsupported logos, products, or interfaces with unbranded human/environmental storytelling. Deterministic validation is negation-aware, so instructions such as “no logo” are not mistaken for asset requests. Brand Reels assign every scene a distinct editorial job—pattern interrupt, tangible mechanism/proof, and premium payoff/CTA, with a separate consequence beat in four-scene plans. If the model's schema repair remains incomplete, a bounded local creative rescue preserves valid scenes, distributes project timing across the selected scene count, fits narration, repairs or safely declares cast continuity, rotates reliable shot devices, and supplies the final source-safe audience action. This schema recovery happens before Auto retry logic and is shared by the manual Storyboard button. If a candidate still violates grounding, one bounded model recovery pass rewrites it; a deterministic safe-text fallback removes any residual unsupported visual or claim language before final validation. Only a candidate that still fails after all recovery layers stops for human review. The successful recovery method and omitted capabilities are persisted in the storyboard job output and explained in the editor. The resulting storyboard creates separate product/character/visual-world continuity locks, classifies continuity as continuous, match-cut, or intentional change, and separately chooses a reviewable transition style (`CUT`, `FADE`, `SLIDE`, `WIPE`, `IRIS`, or `CLOCK_WIPE`) for each incoming scene. When the selected concept has a live raster opening frame, Storyboard creates Scene 1's selected `KEYFRAME_START` take by referencing that same artifact. Scene 1 therefore incurs no second image-generation call; normal anchor generation starts at Scene 2.
...
12. Render job uses Remotion to sequence each narration artifact inside its owning scene and add short audio fades. Every output mode can resolve the storyboard's AI-selected curated BGM id (or a manual Final-step override), hash-version the public MP3 into the project artifact store, loop it, fade its reel edges, and duck it during speech. Product Showcase defaults this layer on; an explicit user false remains authoritative for a voiceover-only re-render. Legacy project-wide narration and null-track storyboards remain supported; the latter are mood-matched from their saved direction. The official Remotion transition presentations are selected from persisted scene intent; sequence timing absorbs each short overlap so the exported duration still equals the storyboard total. Only the last scene receives a closer/CTA caption: the verified logo owns the full closing shot, while the closure copy enters at its midpoint so the first half remains unobstructed video. Before Chromium starts extracting frames, every source video, narration track, music bed, and logo is downloaded once with bounded retries and signature-checked. Remote durable artifacts are fetched directly; local artifacts traverse the same-origin endpoint once. A dedicated loopback-only HTTP server then serves the staged buffers with correct MIME, CORS, HEAD, and byte-range behavior for the duration of the render. Remotion therefore never performs repeated frame/audio seeks through the busy Next.js development server, Prisma, or remote OSS. Rendering retains a 120-second component timeout and conservative concurrency, then produces the final 9:16 MP4 and thumbnail. The latest uploaded `LOGO` image—or a directly discovered website asset explicitly labeled as a logo/wordmark—is rendered with Remotion's image component after staging validation. This prevents progressive or partially decoded image scanlines from entering the export. The logo is composited alone at three times the previous mark size; the business name is used only when no verified logo asset is available. Generative image/video prompts continue to prohibit synthetic logo drawing.
...
- Storyboards are fully editable field by field after generation, including their global continuity bible and per-scene transition modes. Full-plan regeneration remains available, while each approved scene exposes a clearly labeled frame action; Scene 1 calls it the opening frame and stays synchronized with the selected concept preview.
- Production is additive and modular without making history the primary workflow: every scene exposes independent frame and clip actions, including regeneration of already successful outputs. Attempts create takes rather than destructively replacing prior artifacts, the newest complete result becomes the recommended path automatically, and the collapsed version history can restore any completed prior frame or clip. Changing the one shot sentence or continuity metadata invalidates that scene and downstream anchors/clips; timing-only changes invalidate only that clip. A selected-take change marks only completed final renders stale. Final render validates the entire ordered storyboard, uses direct cuts without per-scene fade-from-black, and never exports a filtered subset of completed scenes.
