...
9. Storyboard generation produces one `shotPrompt` per scene under a strict contract: exactly one mood-first sentence of 8–36 words, one primary subject, one action, one reliable camera behavior, no sequential action language, and a 5–10 second duration. Continuity mode/notes and the global continuity bible remain engine metadata rather than competing model-facing prose. Keyframe jobs derive one high-resolution scene anchor from the shot sentence and chain prior anchors as references to preserve identity, spatial logic, screen direction, lighting, and edit geometry. A refreshed chain is activated atomically only after every anchor succeeds; prior anchors and legacy closing-frame takes remain history.
