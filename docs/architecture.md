...
9. Keyframe jobs generate and store one recommended scene anchor per scene in OSS. Provider-accessible product/logo/reference imagery supplies brand grounding. For every later continuous or match-cut scene, the previous generated anchor plus its motion and natural-exit brief guide the next anchor's identity, spatial logic, screen direction, lighting, and edit geometry. Intentional-change transitions break only the requested story attributes while retaining the continuity bible. A refreshed anchor chain is activated atomically only after every new anchor succeeds; prior anchors and legacy closing-frame takes remain low-emphasis history.
10. Video jobs preflight each explicitly selected anchor, submit it as the sole still-image constraint through the Wan 2.7 i2v protocol, and include the next scene's anchor context in the motion brief. There is no `last_frame` constraint, so intermediate clips maintain purposeful motion through an edit-safe exit and the final scene resolves naturally. Healthy sibling tasks continue polling when one scene fails, transient submission failures receive bounded retries, and the Production UI can resubmit only a missing/failed scene without spending again on completed siblings. An unsuccessful retry preserves the prior selected clip. Production starts and poll claims are serialized to prevent duplicate provider spend and artifacts.
