...
5. At render time the chosen public WAV is hashed and copied into the normal project artifact store. Replacing a public file therefore creates a fresh artifact on the next render while unchanged tracks are reused.
...
| ---------------------- | ---------------- | --------------------------------------------- |
| `warm-uplift.wav`      | Warm Uplift      | services, testimonials, community, lifestyle  |
| `clean-momentum.wav`   | Clean Momentum   | technology, SaaS, explainers, business        |
| `bold-kinetic.wav`     | Bold Kinetic     | sports, fashion, events, product launches     |
| `cinematic-wonder.wav` | Cinematic Wonder | luxury, travel, property, emotional reveals   |
| `calm-organic.wav`     | Calm Organic     | wellness, food, beauty, craft, sustainability |

The committed files are valid eight-second synthetic placeholders, not production music. Export the replacements as PCM WAV, preferably 44.1 or 48 kHz, stereo, 30–60 seconds, and designed to loop cleanly. Master all five to a similar perceived loudness (about -14 LUFS integrated, no higher than -1 dBTP) and leave the vocal midrange uncluttered. Keep proof that the generated or licensed tracks permit commercial use.

## Generation prompts

These prompts are intentionally instrumental and voiceover-friendly. If the music tool has separate controls, choose **instrumental**, disable lyrics/vocals, request a seamless loop, and export WAV.
...
5. Temporarily replace one WAV with another valid WAV using the same filename, render again, and confirm the replacement is heard; the content hash prevents stale artifact reuse.
