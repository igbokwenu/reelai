...
- Near-valid storyboard output is repaired deterministically before persistence: the first clear action is preserved, short mood fragments are normalized, supported camera direction survives sentence cleanup, Product Showcase timing and narration are fitted to the exact project target, and an omitted camera instruction falls back to a stable fixed-camera setup. Missing creative direction still fails validation.
