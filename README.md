...
<a href="https://youtu.be/tl69m80pK7k" target="_blank" rel="noopener noreferrer">
  <img src="docs/demo-thumbnail.png" alt="Watch the Reel AI demo" width="100%"/>
</a>
...
- Optional **Razzmatazz mode** turns Product Showcase into a fixed 5-second commercial bumper: one intact-product hero scene with explicit product motion, animated surrounding light/particle/reflection energy, unmistakable center-of-attention framing, no people or teardown, and a short tagline/CTA revealed over the second half. Auto mode and step-by-step mode share the same guardrails and exact 150-frame Remotion export.
...
- Visual storyboard filmstrip with one anchor image and one directed shot sentence per scene, an engine-managed product/character/visual-world continuity bible, and a human approval loop. Short-format defaults are Razzmatazz 5s/1 locked scene, standard Showcase 5s/1 scene, 10s/2 scenes, and 15s/3 scenes; explicit supported scene-count directions override the default except for locked Razzmatazz. Standard reels otherwise use 2–4 scenes, while Product Showcase uses a feasible 1–3 scenes at the exact selected duration.
...
- Near-valid storyboard output is repaired deterministically before persistence: the first clear action is preserved, multi-sentence prose is collapsed safely, conflicting camera language becomes one supported behavior, and Product Showcase timing and narration are fitted to the exact project target. Brand Reel concepts and storyboards receive one model schema-repair pass followed by a bounded local creative rescue when needed; that rescue preserves usable decisions, completes exactly three differentiated directions, restores timing/copy/cast/continuity contracts, and gives multi-scene reels a hook → proof → payoff spine without surfacing the model's own formatting misses to the user. Recovery method metadata is retained on the generation job for diagnosis.
...
- Before Remotion starts, every source video, narration track, music bed, and logo is downloaded once with bounded retries, signature-checked, and staged behind a dedicated loopback-only byte-range server. Frame extraction and audio decoding therefore do not repeatedly traverse Next.js, Prisma, or remote OSS while the render is consuming CPU. Remote durable artifacts are fetched directly for staging; local artifacts use the app endpoint once. Render concurrency remains conservative for stable local exports.
...
- Final renders keep early scenes text-free and reserve the first half of the closing shot for unobstructed video plus the verified logo. The closer/CTA enters smoothly at the midpoint in a restrained glass-and-pearl typographic treatment, including single-clip 5-second Razzmatazz and standard showcases. The latest uploaded logo, or a directly verified website logo asset—not an AI recreation—is displayed alone at 3× the former mark size; the business name appears only when no logo asset is available.
...
Razzmatazz mode adds the persisted `Project.razzmatazzMode` preference, and the 5-second upgrade includes a follow-up data migration for any Razzmatazz projects created under the former 3-second contract. Stop `pnpm dev`, run `pnpm db:migrate`, then restart `pnpm dev`. No package install, seed, or manual Prisma generation is required. Existing non-Razzmatazz projects are unchanged; existing Razzmatazz projects are moved to 5 seconds and can be tested by regenerating their concepts/storyboard.
...
The Brand Reel internal creative-recovery and multi-scene editorial-quality update is application-only. Restart `pnpm dev`; do not run `pnpm install`, `pnpm db:migrate`, `pnpm db:generate`, or `pnpm db:seed`. Existing Brand Reel projects, selected concepts, failed Auto runs, storyboards, and completed media remain compatible. Retry the failed phase on an existing project to use the new recovery path, or replace its three concepts to exercise the complete concept-to-final lifecycle.
