...
- Visual storyboard filmstrip with one anchor image and one directed shot sentence per scene, an engine-managed product/character/visual-world continuity bible, and a human approval loop. Short-format defaults are 5s/1 scene, 10s/2 scenes, and 15s/3 scenes; explicit supported scene-count directions override the default. Standard reels otherwise use 2–4 scenes, while Product Showcase uses a feasible 1–3 scenes at the exact selected duration.
