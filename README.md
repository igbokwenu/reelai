...
- Standard reels use a five-track curated BGM catalog. The storyboard agent selects a mood-matched track, existing projects receive a deterministic fallback, Auto mode honors it, and the Final step provides inline preview plus manual override. Remotion loops, edge-fades, and narration-ducks the chosen bed; Product Showcase remains voiceover-only. See [docs/background-music.md](docs/background-music.md) for filenames, generation prompts, licensing guidance, and tests.
